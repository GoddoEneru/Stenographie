...
def convert_from_text_to_binary(text):
    return "".join(format(ord(x), "b") for x in text)


def convert_from_binary_to_text(binary_text):
...
def steganography_image(image, text):
    image_with_coeff_even = make_the_coeff_of_image_even(image)
    binary_text = convert_from_text_to_binary(text)
